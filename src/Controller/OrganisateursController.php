...
class OrganisateursController extends AbstractController
{
    ...
    /**
     * @Route("/json", name="organisateur_json", methods={"GET"})
     */
    public function organisateurjson(OrganisateursRepository $organisateurRepository,SerializerInterface $serializerInterface  ):response
    {
        $organisateur = $organisateurRepository->findAll();
        $jsonContent= $serializerInterface->serialize($organisateur,'json',['groups'=> 'organisateur']  );
        return new Response($jsonContent);
    }

    /**
     * @Route("/addOrganisateurJSON/new", name="add_organisateur_json", methods={"GET"})
     */
    public function addorganisateurJSON(Request $request,SerializerInterface $serializerInterface  )
    {
        $entityManager = $this->getDoctrine()->getManager();
        $organisateur = new Organisateurs();
        $organisateur->setNomOrganisateur($request->get('nom_organisateur'));
        $organisateur->setNumTel($request->get('num_tel'));
        $organisateur->setDescription($request->get('description'));
